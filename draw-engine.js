(() => {
  ...
  function assertCompetitionShape(comp) {
    ...
    if ((comp.teams.length * comp.potCount * comp.opponentsPerPot) % 2 !== 0) {
  ...
  function placeEdge(maps, team, candidate, edges) {
    ...
    associations.get(team.name).set(
      candidate.country,
      associationCount(associations, team.name, candidate.country) + 1
    );
    associations.get(candidate.name).set(
      team.country,
      associationCount(associations, candidate.name, team.country) + 1
    );
  ...
  function candidateScore(comp, maps, team, candidate) {
    ...
    const assocPressure = associationCount(maps.associations, team.name, candidate.country)
    ...
    return reverseCandidates * 100 + reverseNeed * 8 + assocPressure * 5 + Math.random() * 3;
  }
  ...
  function orientEdges(comp, edges) {
    ...
    for (const team of comp.teams) {
      ...
      for (const potGroup of groups) {
        ...
        const firstNegation = teamIsSecond(edges[firstEdgeIndex], team);
        const secondNegation = teamIsSecond(edges[secondEdgeIndex], team);
        const parity = 1 ^ firstNegation ^ secondNegation;
  ...
  function orderFixtures(fixtures, comp) {
    const byPot = new Map();
    for (let pot = 1; pot <= comp.potCount; pot += 1) {
      byPot.set(pot, shuffle(fixtures.filter((fixture) => fixture.pot === pot)));
    }
    const ordered = [];
    for (let round = 0; round < comp.opponentsPerPot; round += 1) {
      for (let pot = 1; pot <= comp.potCount; pot += 1) {
        const fixture = byPot.get(pot)[round];
  ...
  function createTable(comp, edges, orientation) {
    const table = Object.fromEntries(comp.teams.map((team) => [team.name, []]));
    edges.forEach((edge, index) => {
      const aHome = orientation[index] === 1;
      table[edge.a.name].push({ opponent: edge.b, pot: edge.b.pot, home: aHome });
      table[edge.b.name].push({ opponent: edge.a, pot: edge.a.pot, home: !aHome });
    });
    comp.teams.forEach((team) => { table[team.name] = orderFixtures(table[team.name], comp); });
    return table;
  }
  ...
  function validateCompetitionDraw(comp, table) {
    ...
    for (const team of comp.teams) {
      ...
      if (new Set(opponentNames).size !== opponentNames.length) {
        return { valid: false, reason: `${team.name}: tekrar eden rakip var.` };
      }
      if (fixtures.some((fixture) => fixture.opponent.country === team.country)) {
        return { valid: false, reason: `${team.name}: aynı ülkeden rakip var.` };
      }
      const associationCounts = {};
      fixtures.forEach((fixture) => {
        associationCounts[fixture.opponent.country] = (associationCounts[fixture.opponent.country] || 0) + 1;
      });
      if (Object.values(associationCounts).some((count) => count > 2)) {
        return { valid: false, reason: `${team.name}: federasyon sınırı aşıldı.` };
      }
      for (let pot = 1; pot <= comp.potCount; pot += 1) {
      ...
      for (const fixture of fixtures) {
        ...
        if (!reciprocal || reciprocal.home === fixture.home) {
          return { valid: false, reason: `${team.name}: karşılıklı eşleşme tutarsız.` };
        }
      }
    }
  ...
  function generateCompetitionDraw(comp, options = {}) {
    ...
    for (let orientationAttempt = 0; orientationAttempt < 40; orientationAttempt += 1) {
      const edges = buildEdges(comp, maxAttempts);
      const orientation = orientEdges(comp, edges);
      if (!orientation) continue;
      const table = createTable(comp, edges, orientation);
      const validation = validateCompetitionDraw(comp, table);
      if (validation.valid) return table;
    }
    throw new Error('Kura eşleşmeleri üretildi ancak iç saha/deplasman dengesi çözülemedi. Tekrar dene.');
  }

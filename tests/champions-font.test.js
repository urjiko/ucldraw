...
function readFont(prefix) {
  const expectedPartLengths = prefix === 'regular' ? [6000, 6000, 6000, 4936] : [6000, 6000, 6000, 5084];
  const parts = [1, 2, 3, 4].map((part) => {
    ...
    assert.equal(match[1].length, expectedPartLengths[part - 1], `${prefix} font parçası ${part} uzunluğu yanlış`);
    return match[1];
  });
...
if (mode === 'regular' || mode === 'all') {
  ...
  assert.equal(regular.readUInt32BE(8), regular.length);
  console.log('Champions regular WOFF2 bytes passed.');
}

if (mode === 'bold' || mode === 'all') {
  const bold = readFont('bold');
  assert.equal(bold.subarray(0, 4).toString('ascii'), 'wOF2');
  assert.equal(bold.length, 17312);
  assert.equal(bold.readUInt32BE(8), bold.length);
  console.log('Champions bold WOFF2 bytes passed.');
}
...
if (mode === 'ui' || mode === 'all') {
  ...
  console.log('Champions webfont UI integration passed.');
}
